  ...
  return (
    <div className="h-full lg:flex lg:justify-center lg:items-center">
      <div className="px-3 md:lg:xl:px-40 bg-opacity-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 lg:grid-rows-3 xl:grid-cols-4 xl:grid-rows-3 group shadow-xl shadow-neutral-100 border overflow-y-auto h-fit">
          {skills.map(({ id, title }) => (
            <div
              key={id}
              className="p-10 flex flex-col items-center text-center group hover:bg-slate-50 cursor-pointer"
            >
              <span className="p-5 rounded-full bg-red-500 text-white shadow-sm shadow-red-200"></span>
